...
│   │   ├── models/            # 数据模型
│   │   ├── schemas/           # Pydantic 模式
│   │   ├── services/          # 业务服务（RAG 核心 + 文档处理流水线）
│   │   │   ├── document_parser.py       # 智能文档解析（PDF/Word/TXT + OCR）
│   │   │   ├── document_preprocessor.py # 文档预处理（剔除封面/目录/页眉页脚）
│   │   │   ├── document_cleaner.py      # 三级清洗流水线
│   │   │   ├── ocr_service.py           # PaddleOCR 集成（扫描版 PDF）
│   │   │   ├── document_processor.py    # 文档处理编排（预处理→解析→清洗→分块→向量化）
│   │   ├── tasks/             # Celery 异步任务
│   │   └── main.py            # FastAPI 入口
│   ├── scripts/               # 维护脚本
│   ├── tests/                 # 测试文件
...
> **生成层评测指标说明**（pytest 评测框架）：
> | 指标 | 含义 | 通过阈值 |
> |------|------|---------|
> | grounded (Jaccard) | 答案分词与检索上下文 token 重叠率（过滤停用词） | easy≥0.40, medium≥0.30, hard≥0.20 |
> | 关键词命中率 | 期望关键词在答案中的命中率（精确+分词回退） | ≥ 0.25 |
> | 内容覆盖率 | 预期内容片段的 4-gram 核心短语在答案中的命中比例 | easy≥0.20, medium≥0.15, hard≥0.10 |
> | latency | /ask + /eval-retrieve 接口响应时间 | ≤ 35s |
> | rejection | 知识库无结果时是否正确拒答 | 全部通过 |
> | 多轮对话 | 代词指代/省略主语/递进追问的上下文关联 | kw ≥ 0.30 |
> | 流式输出 | SSE chunk 拼接 = done 答案，首字节 < 10s | 全部通过 |
>
> **注意**：hard 问题 LLM 大幅改写原文，内容覆盖率自然偏低。当 Jaccard ≥ 0.30 且关键词命中率 ≥ 0.50 时，豁免内容覆盖率检查。
>
> **注意**：
...
| 指标 | 数值 | 说明 |
|------|------|------|
| **总体通过率** | 100% (29/29) | 至少命中 1 条预期关键词 |
| **EASY（9 题）** | 100.0% (9/9) | 校训、注册、作弊处分等常识性问题 |
| **MEDIUM（12 题）** | 100.0% (12/12) | 奖学金、缓考、转专业等流程性问题 |
| **HARD（8 题）** | 100.0% (8/8) | 退学试读、双学位、最长学制等细节性问题 |
| **关键词召回率** | 0.862 | 命中的关键词数 / 预期关键词总数 |
| **MRR@k** | 0.829 | 首个相关结果的平均倒数排名 |
| **Top-K 精准率** | 0.460 | 含关键词的 chunk 数 / 返回的 chunk 总数 |

> 全部 29 个问题均能正确召回相关文档，关键词召回率 0.862 表明检索覆盖度良好。MRR@5 0.829 表明相关文档排序质量较高。
...
| 指标 | 数值 | 说明 |
|------|------|------|
| **总体通过率** | 100% (12/12) | 满足所有评测维度阈值 |
| **扎根度(Jaccard)** | avg=0.811  min=0.588  max=0.929 | 答案与检索上下文的词重叠度 |
| **扎根度(Embedding)** | avg=0.684  min=0.530  max=0.837 | 答案与检索上下文的语义相似度 |
| **平均延迟** | 22.1s  max=33.1s | 端到端响应时间 |
...
| 内容覆盖 | 100% (12/12) | 覆盖预期内容片段 |
| 延迟 | 100% (12/12) | 响应时间 ≤ 35s |
| 来源引用 | 100% (12/12) | 正确标注信息来源 |

**分难度统计：**
| 难度 | 通过/总数 | 扎根度(avg) | Embedding(avg) |
|------|-----------|-------------|----------------|
| Easy | 4/4 | 0.719 | 0.589 |
| Medium | 4/4 | 0.870 | 0.705 |
| Hard | 4/4 | 0.843 | 0.756 |
...
- **文档预处理**：上传后自动识别并剔除封面页、目录页、版权声明、参考文献、附录、页眉页脚等无效内容
- **三级清洗流水线**：
  1. 全文粗洗：特殊空白字符清理、冗余换行合并、装饰性符号移除
  2. 父块精洗+校验：页码/页眉页脚剔除、模板化文案清理、汉字间空格修复、异常断行合并
  3. 子块轻校验：非空 + 最小长度校验
- **智能文档解析**：自动检测 PDF 类型（可编辑/扫描/混合），扫描版自动调用 PaddleOCR + PP-StructureV2 版面分析进行 OCR 识别，低质量页面（识别率 < 60%）直接拦截
- **大文件拆分**：超过 50MB 的文件自动拆分为多子文档并行处理，`split_group_id` 关联确保跨文档上下文完整
- **异步任务友好**：支持进度回调、事务回滚、缓存清理、错误重试
