...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│  5. 重排序       │ ← DashScope Reranker API / 启发式降级
└────────┬────────┘
         │
         ▼
┌─────────────────┐
...
- 基于 BM25 算法的关键词检索
- **全量文档索引**：在系统启动时构建一次全量 BM25 索引，后续复用（非惰性按需构建）
...
| 层级 | 大小 | 重叠 | 用途 |
|------|------|------|------|
| **父块** | 最大 800 字符 | 100 字符 | 返回给 LLM 作为上下文 |
| **子块** | 最大 200 字符 | 30 字符 | 向量检索的最小单元 |

- **分块策略**：优先按段落、标题分块，分隔符优先级依次为：`\n\n` > `\n` > `。` > `！` > `？` > `；` > `，`
