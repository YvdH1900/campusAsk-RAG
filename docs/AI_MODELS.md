...
============================================================
  ...
  关键词召回率: 0.862    MRR@k: 0.829    Top-K 精准率: 0.460
============================================================
```
...
| **关键词召回率** | 0.862 | 在 top_k=5 的限制下，平均能覆盖约 86% 的预期关键词。得益于文档预处理和清洗流水线，检索覆盖度大幅提升 |
| **MRR@k** | 0.829 | 排序质量良好，相关文档通常排在前 1~2 位。`text-embedding-v4` 对校园制度类文本的语义匹配能力稳定 |
| **Top-K 精准率** | 0.460 | 返回的 5 个 chunks 中约有一半包含预期内容，剩余 chunk 虽不含预期关键词但通常为相关上下文，对 LLM 生成答案仍有价值 |

> **结论**：`text-embedding-v4 + dimension=1024` 在当前校园知识库场景下检索质量可靠，全部 29 个标注问题均能正确召回相关文档。关键词召回率 0.862 表明检索覆盖度优秀，MRR@k 0.829 排序能力良好。
