...
**AI 模式**（需手动开启）：调用 LLM 判断答案是否基于检索上下文
```python
# 输入：问题 + 上下文 + 答案
...
> ⚠️ **当前状态**：答案验证默认关闭（`use_answer_verification = False`，`use_ai_verification = False`）。
> AI 模式依赖 LLM 输出的严格格式解析（第一行必须"是/否"，第二行必须"有/无"），
> 实际使用中 LLM 输出格式多变，容易导致良答案被误判为"答案可能存在问题"。
> 规则模式的中文分词算法（空格分词 + 3-gram）也不适用于中文，
> 未来版本优化后会重新启用。
...
| **答案验证** | LLM | 与回答生成共用 LLM 模型 | 无需额外配置（默认禁用） |
...
| 答案验证 | LLM 判断准确性 | 关键词匹配 + 覆盖率计算（默认禁用 AI 模式） |
